...
class ControlState:
    ...
    image_height: int = 0
    image_width: int = 0
    # Provenance of the measured region. ``mask_area_ratio`` is meaningless
    # without its denominator, and "the frame" is not the same denominator as
    # "the imaged sector" -- logging the mode and the pixel count makes a state
    # from one acquisition geometry comparable with one from another.
    roi_mode: str = "full"
    roi_area_px: int = 0
