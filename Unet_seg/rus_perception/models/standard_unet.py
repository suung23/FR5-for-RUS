...
#:
#: Note the deliberate asymmetry in ``in_channels`` / ``out_channels``:
#:
#: * ``original`` omits them, so the class defaults (3 in, 2 out) apply. That is
#:   what makes it reproduce upstream ``unet.UNet(3, 2)`` exactly, which is the
#:   whole point of the preset and what ``test_models.py`` checks.
#: * ``paper`` pins them to 1 and 1. The reported 8,635,809-parameter baseline is
#:   a grayscale-in, single-logit-out network; inheriting the Carvana defaults
#:   silently gives 8,636,418 instead (+576 in the first convolution, +33 in the
#:   output convolution) and the comparison against the paper stops being a
#:   comparison. Pinning them here means ``from_preset("paper")`` alone is
#:   correct, matching ``SLIM_UNET_PRESETS``, which has always pinned both.
