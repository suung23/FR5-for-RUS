...
#### 수집 계획 — GUI 가 "지금 무엇을, 몇 번째로" 를 안내 (`collect_plan_<task>.json`, 2026-09-10 2차)

1차 100 에피소드를 분석하니 정책이 배우는 두 축(x, θz)이 **단독으로** 나온 구간이 적고(횡이동만 89, 회전만 43), 복합 이동은
같은 부호(++/−−)로만 묶여 있었다(86 대 15) — 그대로 학습하면 "x 를 움직이면 같은 쪽으로 돈다" 는 가짜 상관을 배운다. 그래서
2차부터는 **에피소드마다 한 종류의 움직임만** 쓰고, 종류별 목표 수는 부호×크기 bin 당 60 구간 이상이 되는 값으로 잡았다
(bin 평균을 라벨 잡음 σ/√60 ≈ 0.1 mm 로 추정, 축별 검증 200 구간이면 부호 정확도 95 % ±3 %).

`start_collect_win.cmd manual find_bladder` 가 `imu_bench\collect_plan_find_bladder.json` 이 있으면 자동으로 `--plan` 으로 넘긴다.
패널에 "수집 계획 k/N — 종류별 진행", "▶ 다음: [i] 종류 — 몇 번째 · 이번 bin(부호·크기)", 규약 한 줄이 뜨고, 저장할 때마다
메타 `episode.motion / bin_hint / index_in_kind` 가 남아 GUI 를 껐다 켜도 진행 수가 이어진다. 종류는 계획 순서대로 블록으로
진행하며 목표를 채우면 자동으로 다음 종류로 넘어간다. **N** 다음 종류, **숫자 키** 로 종류 직접 선택. bin 은 부호가 번갈아
제안되므로 (+x, −x, …) 그대로 따르면 좌우가 저절로 균형 잡힌다. bin_hint 는 제안일 뿐이고 실제 움직임은 라벨러가 IMU 로 잰다.

| 종류 (`motion`) | 규약 | 목표 |
|---|---|---|
| `lateral_x` 횡이동만 | 방광이 좌/우 가장자리에 걸치게 시작 → 회전 없이 x 로만. ±5–10, ±10–20, ±20–30 mm | 75 |
| `rotation_z` 회전만 | 방광이 기울어 보이게 시작 → 접촉점을 축으로 회전만. ±3–7°, ±7–15° | 55 |
| `combined_mixed` 복합(반대 부호) | (+x, −θ), (−x, +θ) 동시에 | 25 |
| `sweep_y` 면외 스윕 | 방광을 관통해 1–2 cm/s 등속, 멈추지 않고 완전히 지나칠 때까지 (오프셋 필터 σ_q·Q_area 곡선용) | 30 |
| `search_free` 자유 탐색 | 1차처럼 — 검증·시험 여유분 | 15 |

공통: 정지 ≥ 0.5 s, 한 번의 이동 ≤ 1.2 s, 방광 중앙에서 1~2 s 정지 후 R. 안 보이는 시작은 30 % 이하, 그때는 +x 먼저 훑고 없으면
회전 (규칙이 없으면 정책이 좌우 양봉 분포를 배워야 해서 데이터가 몇 배 든다). 계획 파일의 숫자·문구는 그냥 고쳐 쓰면 된다.
