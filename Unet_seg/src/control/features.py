...
between them) into a fully populated :class:`~src.control.state.ControlState`.

All geometry follows the convention documented in
:data:`~src.control.state.COORDINATE_CONVENTION`: origin top-left, ``x`` right,
...
def binary_mask_geometry(mask: np.ndarray) -> dict[str, Any]:
    ...
    return {
        ...
        # Pixel centres map to [0, 1]; matches src.losses.temporal.soft_centroid.
        ...
        "mask_area_px": area,
        "mask_area_ratio": area / float(height * width),
    }


def segmentation_confidence_from_probability(probability_map: np.ndarray) -> float:
    """Mean binary certainty ``mean(|2p - 1|)`` over the whole frame, in ``[0, 1]``.
    ...
    if probability_map.size == 0:
        return 0.0
    return float(np.abs(2.0 * probability_map - 1.0).mean())


def boundary_entropy(
    probability_map: np.ndarray, mask: np.ndarray, band_px: int = 2
) -> float:
...
def border_contact_ratio(mask: np.ndarray) -> float:
    """Fraction of the mask's perimeter that lies on the image border.

    A bladder that is partly outside the field of view has a large share of its
    outline on the image edge, which is exactly the situation in which a centroid
    is a biased estimate of the true anatomical centre.
...
def lumen_contrast(
    image: np.ndarray, mask: np.ndarray, ring_px: int = 6
...
def warp_mask_with_flow(mask: np.ndarray, flow_backward: np.ndarray) -> np.ndarray:
    ...
    previous frame, matching :func:`src.flow.warp.warp_backward`.
    """
    import cv2
...
def extract_control_state(
    ...
    confidence = segmentation_confidence_from_probability(probability_map)
    entropy = boundary_entropy(probability_map, mask, config.boundary_band_px)
    border = border_contact_ratio(mask)
    ...
    if image is not None:
        ...
        if image_2d.shape == mask.shape:
            lumen_mean, ring_mean, contrast = lumen_contrast(
                image_2d, mask, config.surrounding_ring_px
            )
        else:
            logger.warning(
