...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            default_value="true",
            description=(
                "접촉력 문턱(probe.yaml, 기본 ‖F‖ 1 N)에서 접촉 프로빙으로 전환하고 "
                "그때부터 로봇이 힘을 잡을지. false 면 힘이 얼마가 되든 접근 상한을 "
                "유지하고 힘 유지도 함께 꺼진다 — 전자저울 검증처럼 의도적으로 문턱을 "
                "넘겨 누르면서 teleop 을 계속해야 하는 절차 전용이다. "
                "⚠️ 안전 거동을 끄는 것이며, 남는 층은 힘 한계뿐이다"
            ),
        ),
        DeclareLaunchArgument(
            "backend",
