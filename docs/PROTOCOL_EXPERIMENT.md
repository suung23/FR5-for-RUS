...
> **실험에 칠 명령은 `RUNBOOK_REAL_RUN.md` 에 있다.** 거기 하나만 보고 돌리면 된다.
>
> 이 문서는 남은 둘을 위해 둔다: **한 번만 하는 시스템 검증(V1–V8)** 과, 각 값이 왜
> 그 값인지. 두 곳에 같은 명령을 적어 두면 한쪽만 고쳐지고, 조작자는 어느 쪽이 맞는지
> 알 수 없다 — 그래서 실행 경로는 런북 한 곳으로 모았다.


2026-09-11. **무엇을 검증하는지**는 `EVAL_PLAN_POLICY_RESCUE.md`(사전 등록)에, **시스템의
세부**는 `RUNBOOK_POLICY_INFERENCE.md`에 있다. 어긋나면 사전 등록이 이긴다.
