...
**모든 축은 100 Hz admittance 또는 5 Hz policy가 구동합니다. policy는 기구학만 말하고,
힘으로 옮기는 것은 제어기 몫입니다** (§16.2).

| 축 | 구동 | 추종 대상 | 대역 | setpoint 출처 |
|---|---|---|---|---|
| `z` | admittance | `F_n → F_n*` | 100 Hz | Stage 1 탐색 → Stage 2 저대역 (§8.4) |
| `rx` | admittance | `M_x → M*_x` | 100 Hz | policy `Δθ*` → 환산 (§16.2) |
| `ry` | admittance | `M_y → M*_y` | 100 Hz | policy `Δθ*` → 환산 (§16.2) |
| `x`, `y`, `rz` | **policy** | 속도 직접 지령 | 5 Hz | — |

policy 출력은 **기구학 5개**입니다:

```
(d_x, d_y, d_rz)         면내 운동 — 속도로 반영
(d_rx, d_ry)             기울기 — 제어기가 M* 로 환산 후 힘 루프가 추종
```

**policy 는 힘을 출력하지 않습니다.** 전부 기구학이며, 힘으로 옮기는 것은 제어기 몫입니다
(§16.2). `F_n*` 는 policy 가 아니라 규칙 기반 탐색·적응이 정합니다.
...
**채택안이 2026-08-18에 다시 바뀌었습니다.** 시연 데이터가 핸드헬드 프로브 + IMU 로
정해지면서 **힘 라벨이 존재하지 않게 되었기 때문입니다** (§16.2). policy 는 배우지 않은 것을
출력할 수 없습니다.

| 단계 | `F_n*` 출처 | 성질 |
|---|---|---|
| **Stage 1a / 1b** | 명시적 격자 탐색 (§7.3) | 규칙 — `min{F : Q̄ ≥ (1−ε)Q̄max}` |
| **Stage 2** | **저대역 적응 유지** (policy 아님) | 규칙 |

디더 폐기는 그대로 유효합니다 — 귀속 문제는 실재합니다. 대신 Stage 2 의 적응을
**정지 중에만** 수행하도록 게이트를 겁니다. policy 가 `|v_x|, |v_y|` 를 임계 이하로
두는 구간에서만 섭동하므로 이동에 의한 오염이 성립하지 않습니다.

- 쟁점 3 의 "최대 품질을 내는 최소 힘"이 **전 구간에서 규칙으로** 확보됩니다
- Stage 1 이 만든 힘–품질 곡선은 여전히 에피소드에 기록합니다 — 나중에 힘을 학습으로
  옮길 때 필요합니다
- **0.2 Hz 루프는 되살아납니다.** §11 의 타임스케일 표에 다시 들어갑니다
...
| 영상 policy | **~5 Hz** | 속도 3축 `(x, y, rz)` |
| `F_n*` 배경 적응 | **~0.2 Hz** | Stage 2 저대역 루프, **정지 중에만** (§8.4) |

힘 루프(100 Hz)와 영상 루프(5 Hz)가 **20× 벌어져** 깔끔하게 분리됩니다.

`F_n*` 적응은 2026-08-18 에 다시 규칙 기반으로 돌아왔습니다 — 시연에 힘 라벨이 없어
policy 가 학습할 수 없기 때문입니다 (§16.2). 다만 이동 중 섭동은 귀속이 불가능하므로
**정지 게이트**를 겁니다.

Stage 1 의 명시적 격자 탐색은 상태 전이 중 한 번만 도는 절차이지 상시 루프가 아닙니다.
...
# 16. 학습 데이터와 정책 (2026-08-18)

## 16.1 시연 모달리티 ✅

**핸드헬드 프로브 + 9축 IMU.** 술자가 프로브를 손으로 잡고 시연하며, 기록되는 것은
초음파 영상과 같은 시간축의 IMU 값뿐입니다. 로봇은 시연에 관여하지 않습니다.

얻는 것: 술자 접근성, 수집 속도, 자연스러운 조작.
잃는 것: 정확한 pose 와 접촉력 — 아래 두 절이 그 대가를 다룹니다.

## 16.2 힘 라벨이 없다는 것의 귀결 ✅

핸드헬드 리그에는 F/T 센서가 없으므로 `F_n*`, `M*_x`, `M*_y` 의 **라벨이 존재하지
않습니다.** policy 는 배우지 않은 것을 출력할 수 없습니다.

따라서 **2026-08-17 의 두 결정이 되돌려집니다**:

| 8/17 결정 | 8/18 상태 |
|---|---|
| Stage 2 의 `F_n*` 를 policy 가 출력 (§8.4 절충안) | **철회** — 저대역 규칙 적응 유지 |
| policy 가 `M*` 를 직접 출력 | **기울기 `Δθ*` 출력** → 제어기가 `M*` 로 환산 |

디더 폐기 자체는 유효합니다(귀속 문제는 실재). 대신 Stage 2 적응에 **정지 게이트**를
겁니다 — `|v_x|, |v_y|` 가 임계 이하인 구간에서만 섭동하므로 이동 오염이 성립하지 않습니다.

### 모멘트 편향은 살아남습니다 ✅ (2026-08-18 정정)

`M*` 편향(§5.4)이 힘 라벨과 함께 사라지는 것처럼 보였으나, **policy 가 힘을 출력하지
않기 때문에** 문제가 성립하지 않습니다.

> policy 는 **기구학**만 말한다. 힘으로 옮기는 것은 제어기가 한다.

```
policy  →  Δθ*  (목표 기울기)          ← IMU 자이로로 라벨링됨
            ↓
       환산 계층  (제어기 몫)
            ↓
        M*_x, M*_y
            ↓
   admittance 100 Hz    ω = (M* − M) / B_r
```

`d_rx, d_ry` 는 `d_rz` 와 똑같이 자이로로 라벨링되므로 **힘 라벨이 하나도 필요 없습니다.**
policy 출력은 **5개** 가 됩니다: `(d_x, d_y, d_rz, d_rx, d_ry)`.

### 환산 계수를 몰라도 되게 한다 🟡

정적 환산은 `M* = k_θ · Δθ*` 인데 접촉 회전강성 `k_θ` 를 모르고, 조직과 접촉력에 따라
변하므로 상수로 두기 어렵습니다. **적분형 외루프를 쓰면 계수가 필요 없습니다.**

```
M*  ←  M* + K_θ · (θ* − θ_meas)
```

정상상태에서 `θ_meas → θ*` 가 되고, `k_θ` 는 **루프 이득에만** 영향을 줄 뿐 최종
정확도에는 관여하지 않습니다. `θ_meas` 는 추론 시 IMU 와 FK 양쪽에서 얻을 수 있습니다.

대역은 `admittance(100 Hz) ≫ tilt 외루프 ≫ policy(5 Hz)` 순서를 지킵니다 —
🟡 20 Hz 로 돌리되 유효대역 1–2 Hz.

### ⚠️ 그 결과 드러난 것 — `B_r`, `B_z` 가 미검증입니다

```
회전 시상수  τ_θ = B_r / k_θ        B_r = 0.5   k_θ = ⏳ 미측정
병진 시상수  τ   = B_z / k          B_z = 1000  k   = ⏳ 미측정 (2000 가정)
```

`k = 2000 N/m` 은 §7 의 정착 시간을 계산할 때 쓴 **가정값**이고, 거기서 나온 τ = 0.5 s 가
Stage 1 의 32 초 소요를 결정했습니다. `k_θ` 는 아예 모릅니다.

→ 두 강성을 §16.5 실험에서 **함께 측정**합니다. 그래야 `B_z`·`B_r`·정착 시간·Stage 1
소요가 근거를 갖습니다.

## 16.3 IMU 관측 가능성 — 무엇이 라벨링되는가

9축 융합(Madgwick/EKF)이 주는 것과 주지 않는 것을 분리합니다.

| 성분 | 출처 | 품질 |
|---|---|---|
| roll, pitch (중력 대비 절대) | 가속도계 | ✅ 드리프트 없음 |
| **yaw / `rz`** | **자력계** → 자북 | ✅ 드리프트 제거 |
| Δ회전 (짧은 구간) | 자이로 | ✅ 1 s 구간 오차 0.02° 수준 |
| **Δ병진** | 가속도 2중적분 | ❌ **미해결** |

자력계는 `rz` 를 고칩니다. 병진에는 관여하지 않습니다 — 위치는 여전히 가속도
2중적분입니다.

### ⚠️ 느린 운동은 병진에 유리하지 않습니다

반직관적이라 기록해 둡니다. 프로브가 10 mm/s 까지 0.5 s 에 가속하면 실제 가속도는
**약 0.02 m/s²** 입니다. 소비자급 MEMS 바이어스 불안정성은 **0.01–0.1 m/s²** 입니다.

> 신호와 바이어스가 같은 크기이거나 바이어스가 더 큽니다.

빠른 운동은 큰 가속도가 바이어스를 압도하지만 느린 운동은 바이어스에 묻힙니다.
**총 이동이 10 cm 로 작은 것은 도움이 되지 않습니다 — 문제는 총량이 아니라 프레임당
증분이기 때문입니다.**

30 Hz 에서 10 mm/s 면 프레임당 참값은 **0.33 mm** 입니다. 정지 후 경과 시간 `t` 에
대해 바이어스가 만드는 속도 오차 `b·t` 가 증분에 실립니다:

| 경과 `t` | 속도 오차 | 프레임당 오차 | 참값 대비 |
|---|---|---|---|
| 1 s | 20 mm/s | 0.66 mm | **2×** |
| 2 s | 40 mm/s | 1.3 mm | **4×** |
| 5 s | 100 mm/s | 3.3 mm | **10×** |

"천천히 · 10 cm 이하 · 초반은 teleop" 이 유리하게 작동하는 통로는 하나뿐입니다 —
**ZUPT**. 정지 구간(자이로 분산 낮음 + 가속도 크기 ≈ g)을 검출해 속도를 0 으로 리셋하면
누적이 끊깁니다. 다만 2 s 간격 ZUPT 로도 위 표의 4× 지점이라 여유가 크지 않습니다.

## 16.4 라벨 출처 — 축별로 다릅니다

policy 가 내는 3축의 라벨이 각각 어디서 오는지:

| 축 | 라벨 출처 | 품질 |
|---|---|---|
| `d_rz` | IMU 자이로 (+ 자력계 기준) | ✅ |
| `d_x` (면내 lateral) | **광류** — `rus_perception/flow` | ✅ 스케일은 `mm_per_px` |
| `d_y` (면외 elevational) | 스페클 **비상관** | ⚠️ 약함 |
| `d_x`, `d_y` (대안) | IMU 2중적분 | ❓ §16.5 실험 대기 |

`d_y` 가 가장 약합니다. 면외 이동은 광류가 아니라 스페클 비상관으로 추정해야 하는데,
그것이 방광 체적을 훑는 축입니다. 광류에는 **호흡에 의한 조직 변형**도 섞입니다.

리포에는 Farneback / RAFT-small 백엔드와 신뢰도 채점(local NCC, 경계 마스킹)이 이미
있습니다. `flow_reliability` 를 라벨과 **함께 저장**해 실패 구간을 학습에서 배제합니다.

## 16.5 병진 라벨 검증 실험 ⏳

FR5 가 정답을 갖고 있다는 점을 씁니다. **한 번의 실험으로 세 가지가 동시에 정해집니다** —
어느 라벨원을 쓸지, ZUPT 가 필요한지, `mm_per_px` 가 맞는지.

```
IMU 리그를 FR5 플랜지에 고정
  → 로봇이 대표 궤적 재생 (≤10 mm/s, 정지 구간 포함, 총 ≤10 cm)
  → FK 를 ground truth 로 세 라벨원 동시 비교

  ① IMU 적분 (ZUPT 유 / 무)
  ② 광류 (Farneback / RAFT-small + 신뢰도)
  ③ FK  ← 정답
```
...
| 지표 | 기준 | 근거 |
|---|---|---|
| 청크(1 s) 변위 오차 | **< 1 mm** | 10 mm 이동의 10% |
| 프레임(33 ms) 증분 오차 | **< 0.1 mm** | 참값 0.33 mm 의 30% |
| `d_rz` 청크 오차 | < 0.5° | |

같은 실험에서 **두 접촉 강성을 함께 측정합니다** — 팬텀을 눌러 `F–δ` 와 `M–θ` 기울기를
재면 끝입니다. 이것이 있어야 `B_z`·`B_r`·정착 시간·Stage 1 소요가 근거를 갖습니다 (§16.2).

| 측정량 | 결정하는 것 |
|---|---|
| `k` [N/m] | `τ = B_z/k` → 정착 시간 → Stage 1 총 소요 |
| `k_θ` [N·m/rad] | `τ_θ = B_r/k_θ` → 자세 정렬 응답, tilt 외루프 이득 |

결과별 조치:

| 결과 | 조치 |
|---|---|
| IMU 병진 통과 | IMU 단독. 가장 단순 |
| IMU 실패, 광류 통과 | `rz`←IMU, `x`←광류 하이브리드. `y` 는 여전히 약함 |
| 둘 다 실패 | 시연을 **로봇 teleop** 으로 전환 (FK 가 정답, 힘도 함께 얻음) |
...
| 다봉성 | 약함 (CVAE) | 강함 |
| 데이터 요구량 | **낮음** | 높음 |
| 추론 비용 | 1 forward | K denoising |
| 부드러움 | temporal ensembling | 내재적 |

**다봉성이 필요 없기 때문에 ACT 입니다.** 방광이 오른쪽에 치우쳐 있으면 오른쪽으로
갑니다 — 경로가 여러 갈래가 아닙니다. DP 의 강점이 발휘될 자리가 없는데 데이터는 더
요구합니다. 술자 한둘, 에피소드 수백 규모에서는 ACT 가 유리합니다.

### 관측과 행동

⚠️ **학습과 추론의 관측이 같아야 합니다.** 핸드헬드 시연에 없고 로봇 추론에만 있는
신호를 넣으면 분포가 어긋납니다. 그래서:

- **접촉력을 policy 관측에서 제외합니다.** 시연에 없습니다. 저대역 힘 루프가 따로 처리
  하므로 문제되지 않습니다.
- **IMU 를 추론 시에도 프로브에 그대로 둡니다.** 로봇 FK 로 대체하면 잡음 특성이 달라져
  분포가 어긋납니다.

```
관측
├─ image        1×H×W   US B-mode
└─ state        9D
    ├─ gravity           3   프로브 프레임 중력 방향
    ├─ heading           1   자력계 융합 — rz 기준
    ├─ center_error      2   ← U-Net
    ├─ Q_seg             1
    ├─ Q_raw             1
    └─ valid             1

행동   (H, 5)   — 전부 기구학. 힘 성분 없음 (§16.2)
    ├─ d_x    mm    면내 lateral      ← 광류
    ├─ d_y    mm    elevational       ← 스페클 비상관
    ├─ d_rz   rad   영상면 회전        ← 자이로
    ├─ d_rx   rad   기울기 → M*_x 로 환산  ← 자이로
    └─ d_ry   rad   기울기 → M*_y 로 환산  ← 자이로
```

`center_error` 를 명시적으로 주는 것이 핵심입니다. "방광을 가운데로"라는 목표가 이미
그 신호에 인코딩되어 있는데, 희소한 데이터로 픽셀에서 재발견시키는 것은 낭비입니다.
U-Net 을 개선하면 policy 도 함께 좋아진다는 이점도 있습니다.

### 청크

```
예측    H = 30 step @ 30 Hz  =  1.0 s
실행    앞 6 step            =  200 ms
재계획  5 Hz
```

30 Hz 영상과 5 Hz policy 가 정확히 6:1 이라 맞아떨어지고 temporal ensembling 여지도
남습니다.

## 16.7 데이터 형식

같은 녹화에 **주석 레이어가 두 개** 얹히는 구조입니다. U-Net 은 `label_seg` 로,
policy 는 `label_policy` 로 각각 학습합니다.

```
episode_YYYYMMDD_HHMMSS.h5
├─ meta/
│   ├─ mm_per_px          스캐너 깊이 설정 — 광류를 mm 로 바꾸는 데 필수
│   ├─ depth_setting, probe_id, operator_id, patient_id
│   └─ imu_to_probe       IMU→프로브 고정 변환 ⏳ 리그 확정 후
├─ t                      [T]      단일 시계 (하드웨어 타임스탬프)
├─ image                  [T,H,W]  mono8 또는 JPEG
├─ imu/
│   ├─ quat               [T_i,4]  IMU 원래 레이트 (~100–200 Hz)
│   ├─ gyro               [T_i,3]
│   ├─ accel              [T_i,3]
│   ├─ mag                [T_i,3]
│   └─ t_imu              [T_i]
├─ label_policy/                   ← 파생. 재계산 가능하게 원본과 분리
│   ├─ d_x, d_y, d_rz     [T]
│   ├─ flow_reliability   [T]
│   ├─ zupt_flag          [T]      정지 검출 결과
│   └─ gravity, heading   [T,·]    관측 입력용
└─ label_seg/                      ← 희소. 주석한 프레임만
    ├─ frame_idx          [N]
    └─ mask               [N,H,W]
```
...
**① 파생 라벨을 원본과 분리합니다.** 광류 백엔드나 스케일이 바뀌면 재계산해야 하는데,
섞여 있으면 원본까지 다시 녹화해야 합니다.

**② `flow_reliability` 를 반드시 함께 저장합니다.** 광류가 실패한 구간(스페클 결여, 큰
변형)의 병진 라벨은 쓰레기입니다. 학습에서 가중치로 쓰거나 제외해야 합니다.

**③ 시간 정렬은 샘플링이 아니라 적분입니다.**

```
d_rz[k] = ∫ ω_z dt        (t_k → t_{k+1} 구간 적분)
```

IMU 100–200 Hz 를 30 Hz 로 샘플-앤-홀드하면 프레임 사이의 움직임이 사라집니다.
적분해야 델타가 보존됩니다.

⚠️ **하드웨어 타임스탬프가 전제입니다.** IMU 로거와 프레임그래버가 각자 시계를 쓰면
드리프트가 생기고, 이것은 사후 보정이 불가능합니다. §14.3 에 적어둔 근본 결함이 여기서
그대로 터집니다 — **녹화 리그 설계 시점에 해결해야 합니다.**
...
| 2026-08-18 | — | 시연은 **핸드헬드 프로브 + 9축 IMU**. 로봇 미관여. §16.1 |
| 2026-08-18 | — | 힘 라벨 부재 → 8/17 절충안 **철회**. Stage 2 도 저대역 규칙 적응. §16.2 |
| 2026-08-18 | — | **policy 는 힘을 출력하지 않는다.** 기구학 5개 `(x,y,rz,rx,ry)` → 제어기가 `M*` 로 환산. §16.2 |
| 2026-08-18 | — | tilt 환산은 **적분형 외루프** — `k_θ` 를 몰라도 성립. §16.2 |
| 2026-08-18 | — | `B_z`·`B_r` **미검증** 확인. 접촉 강성 `k`, `k_θ` 를 §16.5 실험에서 측정. |
| 2026-08-18 | — | 자력계가 `rz` 드리프트 해소. **병진은 미해결** — 실험 대기. §16.3 |
| 2026-08-18 | — | 학습 방식 **ACT**. 다봉성 불필요 + 데이터 희소. §16.6 |
| 2026-08-18 | — | policy 관측에 **품질 신호 포함**, 접촉력 제외(시연에 없음). §16.6 |
