...
# Float32MultiArray 임포트 추가
from std_msgs.msg import Float32, String, Float32MultiArray
from geometry_msgs.msg import Twist 
# [수정] 4D 상태를 받기 위해 Pose 대신 Float32MultiArray 사용 (발행 측과 타입이 맞아야 함)
# 만약 퍼블리셔가 Float32MultiArray로 [x, y, z, roll]을 쏜다고 가정
...
class SuturingMasterNode(Node):
    def __init__(self):
        ...
        # === [그리퍼 연속 N번 로직 추가] ===
        self.GRIP_CONSECUTIVE_THRESHOLD = 3  # 연속 3번 같은 상태가 예측되어야 실제로 변경
        ...
        # [수정] 불필요한 Joint, Wrt_Right_Gripper 등 제거. 새로운 New RCM 토픽 구독.
        # 주의: 4D 상태 [x, y, z, roll]이 Float32MultiArray로 들어온다고 가정합니다.
        ...
        # 클릭한 Needle Points를 다른 노드에서도 볼 수 있도록 퍼블리시
    ...
    def update_4d_state(self, key, msg):
        """ [NEW] Float32MultiArray로 들어오는 4D [x, y, z, roll] 상태를 업데이트 """
    ...
    def convert_action_to_twist(self, action_sub_vec):
        """ Twist 변환 함수 (Grip은 별도 로직으로 처리, action_sub_vec: [dx, dy, dz, droll]) """
        t = Twist()
    ...
    def run_inference_loop(self):
        ...
        while rclpy.ok():
            ...
            # Check all required data available
            ...
            try:
                ...
                for i in range(len(actions_seq)):
                    ...
                    action_vec = actions_seq[i] # (10,) [dx_l, dy_l, dz_l, droll_l, grip_l, dx_r, dy_r, dz_r, droll_r, grip_r]
                    ...
                    # [델타 제어] 시간이 멈춰있을 때는 제어값을 0(빈 Twist)으로 세팅
